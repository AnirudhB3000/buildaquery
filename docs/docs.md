...
10. [CockroachDB Compiler](#cockroachdb-compiler)
11. [Oracle Compiler](#oracle-compiler)
12. [SQL Server Compiler](#sql-server-compiler)
13. [Execution Layer](#execution-layer)
14. [Traversal Patterns](#traversal-patterns)
...
**Build-a-Query** is a Python-based query builder library designed to programmatically construct, compile, and execute SQL queries using a dialect-agnostic Abstract Syntax Tree (AST). The library supports PostgreSQL, SQLite, MySQL, MariaDB, CockroachDB, Oracle, and SQL Server.
...
- **CockroachDB**: `INTERSECT` / `EXCEPT` are supported (including `ALL`), and `DROP TABLE ... CASCADE` is supported.
...
Abstract Syntax Tree (AST) Layer
    ↓
Compiler Layer (PostgreSQL / SQLite / MySQL / MariaDB / CockroachDB / Oracle / SQL Server)
    ↓
Execution Layer (psycopg / mysql-connector-python / mariadb / oracledb / pyodbc / sqlite3)
    ↓
PostgreSQL / MySQL / MariaDB / CockroachDB / Oracle / SQL Server / SQLite Databases
```
...
- **CockroachDbCompiler**: Implements CockroachDB-specific SQL generation
...
- **CockroachExecutor**: Manages CockroachDB connections via psycopg
...
For CockroachDB, ensure you have a running database and a connection URL (for example: `postgresql://root@host:26257/dbname?sslmode=disable`).
...
The CockroachDB compiler converts AST nodes into executable CockroachDB query strings.

### How It Works

1. **Initialization**: Create a `CockroachDbCompiler` instance
2. **Compilation**: Call `compile(ast_node)` to process the entire tree
3. **Output**: Receive a `CompiledQuery` with SQL string and parameters
...
@dataclass
class CompiledQuery:
    sql: str                    # The CockroachDB query string with %s placeholders
    params: list[Any]          # List of values to be substituted
```
...
- Uses `%s` positional bind variables.
- `TOP` is translated to `LIMIT`.
- `INTERSECT` and `EXCEPT` (including `ALL`) are supported.
- `DROP TABLE ... CASCADE` is supported.
...
The execution layer handles database connections and query execution via psycopg (PostgreSQL/CockroachDB), sqlite3 (SQLite), mysql-connector-python (MySQL), mariadb (MariaDB), oracledb (Oracle), and pyodbc (SQL Server).
...
# Method 2: Existing psycopg connection
import psycopg
conn = psycopg.connect("postgresql://root@localhost:26257/dbname?sslmode=disable")
executor = CockroachExecutor(connection=conn)

# With custom compiler
from buildaquery.compiler.cockroachdb.cockroachdb_compiler import CockroachDbCompiler
executor = CockroachExecutor(
    connection_info="postgresql://root@localhost:26257/dbname?sslmode=disable",
    compiler=CockroachDbCompiler()
)
```
...
The `CockroachExecutor` interface mirrors `PostgresExecutor`:
...
- **Connection String Mode**: Uses a PostgreSQL-compatible URL to open a connection per call.
- **Existing Connection Mode**: Reuses the provided connection.
...
All examples use `PostgresExecutor`, but you can swap in `SqliteExecutor` for SQLite, `MySqlExecutor` for MySQL, `MariaDbExecutor` for MariaDB, `CockroachExecutor` for CockroachDB, `OracleExecutor` for Oracle, or `MsSqlExecutor` for SQL Server by changing the executor import and using the appropriate connection info. Be aware that some functions (e.g., `NOW()`) are dialect-specific and may not exist in SQLite.
...
#### 11. CockroachDB Driver Missing

**Problem**: `No module named 'psycopg'` when running CockroachDB integration tests.
