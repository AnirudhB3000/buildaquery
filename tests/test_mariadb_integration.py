...
def test_basic_crud_lifecycle(mariadb_executor, mariadb_create_table):
    ...
    mariadb_create_table(create_stmt)
    ...
    mariadb_executor.execute(insert_stmt)
    ...
    mariadb_executor.execute(insert_stmt_2)
    ...
    results = mariadb_executor.execute(select_stmt)
    ...
    mariadb_executor.execute(update_stmt)
    ...
    updated_result = mariadb_executor.execute(verify_stmt)
    ...
    mariadb_executor.execute(delete_stmt)

    count_stmt = SelectStatementNode(
        select_list=[FunctionCallNode(name="COUNT", args=[StarNode()])],
        from_table=users_table
    )
    count_result = mariadb_executor.execute(count_stmt)
...
def test_advanced_filtering(mariadb_executor, mariadb_create_table):
    products_table = TableNode(name="integration_products")
    create_stmt = CreateStatementNode(
        table=products_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT AUTO_INCREMENT", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="VARCHAR(255)"),
            ColumnDefinitionNode(name="price", data_type="INT"),
            ColumnDefinitionNode(name="category", data_type="VARCHAR(255)")
        ]
    )
    mariadb_create_table(create_stmt)
    ...
    for name, price, category in data:
        mariadb_executor.execute(InsertStatementNode(
            table=products_table,
    ...
    in_results = mariadb_executor.execute(in_stmt)
    ...
    between_results = mariadb_executor.execute(between_stmt)
    ...
    case_results = mariadb_executor.execute(case_stmt)
...
def test_joins_and_aggregations(mariadb_executor, mariadb_create_table):
    depts_table = TableNode(name="integration_depts")
    mariadb_create_table(CreateStatementNode(
        table=depts_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT AUTO_INCREMENT", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="VARCHAR(255)")
    ...
    mariadb_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="name")], values=[LiteralNode(value="HR")]))
    mariadb_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="name")], values=[LiteralNode(value="Engineering")]))
    ...
    results = mariadb_executor.execute(join_stmt)
    ...
    agg_results = mariadb_executor.execute(agg_stmt)
...
def test_cte_and_window_functions(mariadb_executor, mariadb_create_table):
    sales_table = TableNode(name="integration_sales")
    mariadb_create_table(CreateStatementNode(
        table=sales_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT AUTO_INCREMENT", primary_key=True),
            ColumnDefinitionNode(name="agent", data_type="VARCHAR(255)"),
    ...
    cte_results = mariadb_executor.execute(cte_query)
    ...
    window_results = mariadb_executor.execute(window_stmt)
...
def test_set_operations_and_subqueries(mariadb_executor, mariadb_create_table):
    t1 = TableNode(name="set_t1")
    t2 = TableNode(name="set_t2")

    mariadb_create_table(CreateStatementNode(table=t1, columns=[ColumnDefinitionNode(name="id", data_type="INT")]))
    mariadb_create_table(CreateStatementNode(table=t2, columns=[ColumnDefinitionNode(name="id", data_type="INT")]))

    for i in [1, 2, 3]:
        mariadb_executor.execute(InsertStatementNode(table=t1, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    for i in [3, 4, 5]:
        mariadb_executor.execute(InsertStatementNode(table=t2, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    ...
    union_results = mariadb_executor.execute(union_stmt)
    ...
    intersect_results = mariadb_executor.execute(intersect_stmt)
    ...
    except_results = mariadb_executor.execute(except_stmt)
    ...
    sub_results = mariadb_executor.execute(sub_stmt)
    ...
    sub_from_results = mariadb_executor.execute(sub_from_stmt)
