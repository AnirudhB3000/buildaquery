...
This directory contains integration tests that verify the full query lifecycle—from AST construction and compilation to execution against a live PostgreSQL database.

## Strategy

We use a local Docker-based PostgreSQL instance for integration testing. Each test suite is responsible for its own schema management (creating and dropping tables) to ensure consistency.
