...
def test_basic_crud_lifecycle(mssql_executor, mssql_create_table):
    ...
    mssql_create_table(create_stmt)
    ...
    mssql_executor.execute(insert_stmt)
    ...
    mssql_executor.execute(insert_stmt_2)
    ...
    results = [tuple(row) for row in mssql_executor.execute(select_stmt)]
    ...
    mssql_executor.execute(update_stmt)
    ...
    updated_result = [tuple(row) for row in mssql_executor.execute(verify_stmt)]
    ...
    mssql_executor.execute(delete_stmt)

    count_stmt = SelectStatementNode(
        select_list=[FunctionCallNode(name="COUNT", args=[StarNode()])],
        from_table=users_table
    )
    count_result = [tuple(row) for row in mssql_executor.execute(count_stmt)]
...
def test_advanced_filtering(mssql_executor, mssql_create_table):
    products_table = TableNode(name="integration_products")
    create_stmt = CreateStatementNode(
        table=products_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="NVARCHAR(255)"),
            ColumnDefinitionNode(name="price", data_type="INT"),
            ColumnDefinitionNode(name="category", data_type="NVARCHAR(255)")
        ]
    )
    mssql_create_table(create_stmt)
    ...
    for item_id, name, price, category in data:
        mssql_executor.execute(InsertStatementNode(
            table=products_table,
    ...
    in_results = [tuple(row) for row in mssql_executor.execute(in_stmt)]
    ...
    between_results = [tuple(row) for row in mssql_executor.execute(between_stmt)]
    ...
    case_results = [tuple(row) for row in mssql_executor.execute(case_stmt)]
...
def test_joins_and_aggregations(mssql_executor, mssql_create_table):
    depts_table = TableNode(name="integration_depts")
    mssql_create_table(CreateStatementNode(
        table=depts_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="NVARCHAR(255)")
    ...
    mssql_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="id"), ColumnNode(name="name")], values=[LiteralNode(value=1), LiteralNode(value="HR")]))
    mssql_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="id"), ColumnNode(name="name")], values=[LiteralNode(value=2), LiteralNode(value="Engineering")]))
    ...
    results = [tuple(row) for row in mssql_executor.execute(join_stmt_safe)]
    ...
    agg_results = [tuple(row) for row in mssql_executor.execute(agg_stmt)]
...
def test_cte_and_window_functions(mssql_executor, mssql_create_table):
    sales_table = TableNode(name="integration_sales")
    mssql_create_table(CreateStatementNode(
        table=sales_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT", primary_key=True),
            ColumnDefinitionNode(name="agent", data_type="NVARCHAR(255)"),
    ...
    cte_results = [tuple(row) for row in mssql_executor.execute(cte_query)]
    ...
    window_results = [tuple(row) for row in mssql_executor.execute(window_stmt)]
...
def test_set_operations_and_subqueries(mssql_executor, mssql_create_table):
    t1 = TableNode(name="set_t1")
    t2 = TableNode(name="set_t2")

    mssql_create_table(CreateStatementNode(table=t1, columns=[ColumnDefinitionNode(name="id", data_type="INT")]))
    mssql_create_table(CreateStatementNode(table=t2, columns=[ColumnDefinitionNode(name="id", data_type="INT")]))

    for i in [1, 2, 3]:
        mssql_executor.execute(InsertStatementNode(table=t1, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    for i in [3, 4, 5]:
        mssql_executor.execute(InsertStatementNode(table=t2, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    ...
    union_results = [tuple(row) for row in mssql_executor.execute(union_stmt)]
    ...
    intersect_results = [tuple(row) for row in mssql_executor.execute(intersect_stmt)]
    ...
    except_results = [tuple(row) for row in mssql_executor.execute(except_stmt)]
    ...
    sub_results = [tuple(row) for row in mssql_executor.execute(sub_stmt)]
    ...
    sub_from_results = [tuple(row) for row in mssql_executor.execute(sub_from_stmt)]
