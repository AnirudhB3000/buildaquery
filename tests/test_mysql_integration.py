...
def test_basic_crud_lifecycle(mysql_executor, mysql_create_table):
    ...
    mysql_create_table(create_stmt)
    ...
    mysql_executor.execute(insert_stmt)
    ...
    mysql_executor.execute(insert_stmt_2)
    ...
    results = mysql_executor.execute(select_stmt)
    ...
    mysql_executor.execute(update_stmt)
    ...
    updated_result = mysql_executor.execute(verify_stmt)
...
def test_advanced_filtering(mysql_executor, mysql_create_table):
    ...
    for name, price, category in data:
        mysql_executor.execute(InsertStatementNode(
            table=products_table,
    ...
    in_results = mysql_executor.execute(in_stmt)
    ...
    between_results = mysql_executor.execute(between_stmt)
    ...
    case_results = mysql_executor.execute(case_stmt)
...
def test_joins_and_aggregations(mysql_executor, mysql_create_table):
    depts_table = TableNode(name="integration_depts")
    mysql_create_table(CreateStatementNode(
        table=depts_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="INT AUTO_INCREMENT", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="VARCHAR(255)")
    ...
    mysql_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="name")], values=[LiteralNode(value="HR")]))
    mysql_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="name")], values=[LiteralNode(value="Engineering")]))
    ...
    results = mysql_executor.execute(join_stmt)
    ...
    agg_results = mysql_executor.execute(agg_stmt)
...
def test_cte_and_window_functions(mysql_executor, mysql_create_table):
    ...
    cte_results = mysql_executor.execute(cte_query)
    ...
    window_results = mysql_executor.execute(window_stmt)
    assert window_results[0] == ("A", 100, 2)
    assert window_results[1] == ("A", 200, 1)


# ==============================================================================
# 5. Set Operations & Subqueries (MySQL Compatible)
# ==============================================================================

def test_set_operations_and_subqueries(mysql_executor, mysql_create_table):
    t1 = TableNode(name="set_t1")
    t2 = TableNode(name="set_t2")

    mysql_create_table(CreateStatementNode(table=t1, columns=[ColumnDefinitionNode(name="id", data_type="INT")]))
    mysql_create_table(CreateStatementNode(table=t2, columns=[ColumnDefinitionNode(name="id", data_type="INT")]))

    for i in [1, 2, 3]:
        mysql_executor.execute(InsertStatementNode(table=t1, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    for i in [3, 4, 5]:
        mysql_executor.execute(InsertStatementNode(table=t2, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    ...
    union_results = mysql_executor.execute(union_stmt)
    ...
    sub_results = mysql_executor.execute(sub_stmt)
    ...
    sub_from_results = mysql_executor.execute(sub_from_stmt)
