...
def test_basic_crud_lifecycle(oracle_executor, oracle_create_table):
    ...
    oracle_create_table(create_stmt)
    ...
    oracle_executor.execute(insert_stmt)
    ...
    oracle_executor.execute(insert_stmt_2)
    ...
    results = oracle_executor.execute(select_stmt)
    ...
    oracle_executor.execute(update_stmt)
    ...
    updated_result = oracle_executor.execute(verify_stmt)
...
def test_advanced_filtering(oracle_executor, oracle_create_table):
    products_table = TableNode(name="integration_products")
    create_stmt = CreateStatementNode(
        table=products_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="NUMBER", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="VARCHAR2(255)"),
            ColumnDefinitionNode(name="price", data_type="NUMBER"),
            ColumnDefinitionNode(name="category", data_type="VARCHAR2(255)")
        ]
    )
    oracle_create_table(create_stmt)
    ...
    for item_id, name, price, category in data:
        oracle_executor.execute(InsertStatementNode(
            table=products_table,
    ...
    in_results = oracle_executor.execute(in_stmt)
    ...
    between_results = oracle_executor.execute(between_stmt)
    ...
    case_results = oracle_executor.execute(case_stmt)
...
def test_joins_and_aggregations(oracle_executor, oracle_create_table):
    depts_table = TableNode(name="integration_depts")
    oracle_create_table(CreateStatementNode(
        table=depts_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="NUMBER", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="VARCHAR2(255)")
    ...
    oracle_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="id"), ColumnNode(name="name")], values=[LiteralNode(value=1), LiteralNode(value="HR")]))
    oracle_executor.execute(InsertStatementNode(table=depts_table, columns=[ColumnNode(name="id"), ColumnNode(name="name")], values=[LiteralNode(value=2), LiteralNode(value="Engineering")]))
    ...
    oracle_create_table(CreateStatementNode(
        ...
        columns=[
            ColumnDefinitionNode(name="id", data_type="NUMBER", primary_key=True),
            ColumnDefinitionNode(name="name", data_type="VARCHAR2(255)"),
            ColumnDefinitionNode(name="dept_id", data_type="NUMBER"),
            ColumnDefinitionNode(name="salary", data_type="NUMBER")
        ]
    ))
    ...
    results = oracle_executor.execute(join_stmt_safe)
    ...
    agg_results = oracle_executor.execute(agg_stmt)
...
def test_cte_and_window_functions(oracle_executor, oracle_create_table):
    sales_table = TableNode(name="integration_sales")
    oracle_create_table(CreateStatementNode(
        table=sales_table,
        columns=[
            ColumnDefinitionNode(name="id", data_type="NUMBER", primary_key=True),
            ColumnDefinitionNode(name="agent", data_type="VARCHAR2(255)"),
            ColumnDefinitionNode(name="amount", data_type="NUMBER")
        ]
    ))
    ...
    cte_results = oracle_executor.execute(cte_query)
    ...
    window_results = oracle_executor.execute(window_stmt)
...
def test_set_operations_and_subqueries(oracle_executor, oracle_create_table):
    t1 = TableNode(name="set_t1")
    t2 = TableNode(name="set_t2")

    oracle_create_table(CreateStatementNode(table=t1, columns=[ColumnDefinitionNode(name="id", data_type="NUMBER")]))
    oracle_create_table(CreateStatementNode(table=t2, columns=[ColumnDefinitionNode(name="id", data_type="NUMBER")]))

    for i in [1, 2, 3]:
        oracle_executor.execute(InsertStatementNode(table=t1, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    for i in [3, 4, 5]:
        oracle_executor.execute(InsertStatementNode(table=t2, columns=[ColumnNode(name="id")], values=[LiteralNode(value=i)]))
    ...
    union_results = oracle_executor.execute(union_stmt)
    ...
    intersect_results = oracle_executor.execute(intersect_stmt)
    ...
    except_results = oracle_executor.execute(except_stmt)
    ...
    sub_results = oracle_executor.execute(sub_stmt)
    ...
    sub_from_results = oracle_executor.execute(sub_from_stmt)
