...
def main():
    ...
    db_user = os.getenv("ORACLE_USER", "buildaquery")
    db_password = os.getenv("ORACLE_PASSWORD", "password")

    connection_string = f"oracle://{db_user}:{db_password}@{db_host}:{db_port}/{db_service}"

    # Create executor
    executor = OracleExecutor(connection_info=connection_string)
