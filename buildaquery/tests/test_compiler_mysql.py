...
def test_compile_insert_upsert_duplicate_key_update(compiler):
    ...
    assert (
        compiled.sql
        == "INSERT INTO users (id, name, age) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE name = VALUES(name), age = VALUES(age)"
    )
...
def test_compile_insert_upsert_do_nothing_not_supported(compiler):
    ...
    with pytest.raises(ValueError, match="MySQL upsert does not support do_nothing"):
...
def test_compile_insert_upsert_conflict_target_not_supported(compiler):
    ...
    with pytest.raises(ValueError, match="MySQL upsert does not accept conflict_target"):
        compiler.compile(query)
