...
def test_compile_qualified_names(compiler):
    query = SelectStatementNode(
        ...
        from_table=TableNode(name="users", schema="main")
    )
    compiled = compiler.compile(query)
    assert compiled.sql == "SELECT u.id, u.name FROM main.users"
...
def test_compile_ddl(compiler):
    ...
    expected_create = "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, name TEXT NOT NULL, age INTEGER DEFAULT ?)"
...
def test_compile_drop_cascade_error(compiler):
    ...
    with pytest.raises(ValueError, match="SQLite does not support CASCADE in DROP TABLE"):
