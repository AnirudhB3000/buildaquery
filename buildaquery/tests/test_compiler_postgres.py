...
def test_compile_cast(compiler):
    query = SelectStatementNode(
        select_list=[CastNode(expression=ColumnNode(name="age"), data_type="TEXT")],
        from_table=TableNode(name="users")
    )
    compiled = compiler.compile(query)
    assert compiled.sql == "SELECT CAST(age AS TEXT) FROM users"
...
def test_compile_insert_upsert_do_nothing(compiler):
    ...
    compiled = compiler.compile(query)
    assert compiled.sql == "INSERT INTO users (id, name) VALUES (%s, %s) ON CONFLICT (id) DO NOTHING"
...
def test_compile_insert_upsert_do_update(compiler):
    ...
    assert (
        compiled.sql
        == "INSERT INTO users (id, name, age) VALUES (%s, %s, %s) ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, age = EXCLUDED.age"
    )
...
def test_compile_insert_returning(compiler):
    ...
    compiled = compiler.compile(query)
    assert compiled.sql == "INSERT INTO users (name, age) VALUES (%s, %s) RETURNING id"
    assert compiled.params == ["Alice", 30]

def test_compile_update_returning(compiler):
    query = UpdateStatementNode(
        table=TableNode(name="users"),
    ...
    compiled = compiler.compile(query)
    assert compiled.sql == "UPDATE users SET status = %s WHERE (id = %s) RETURNING id, status"
...
def test_compile_delete_returning_star(compiler):
    ...
    compiled = compiler.compile(query)
    assert compiled.sql == "DELETE FROM users WHERE (id = %s) RETURNING *"
    assert compiled.params == [1]

def test_compile_returning_requires_expression(compiler):
    query = InsertStatementNode(
        table=TableNode(name="users"),
        columns=[ColumnNode(name="name")],
        values=[LiteralNode(value="Alice")],
        returning_clause=ReturningClauseNode(expressions=[]),
    )
    with pytest.raises(ValueError, match="RETURNING requires at least one expression"):
        compiler.compile(query)
